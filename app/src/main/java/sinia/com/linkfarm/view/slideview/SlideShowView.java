package sinia.com.linkfarm.view.slideview;
...
import sinia.com.linkfarm.utils.BitmapUtilsHelp;
...
public class SlideShowView extends FrameLayout {
	...
	public SlideShowView(Context context, AttributeSet attrs, int defStyle) {
		...
		LayoutInflater.from(context).inflate(R.layout.layout_slideshow, this,
				true);
		// uOption = UIL.getOptions(R.drawable.banner1);
	...
	private void initUI() {
		...
		for (int i = 0; i < imageUrls.size(); i++) {
			...
			LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
					AppInfoUtil.dip2px(context, 6), AppInfoUtil.dip2px(context, 6));
			lp.setMargins(AppInfoUtil.dip2px(context, 3), AppInfoUtil.dip2px(context, 0),
					AppInfoUtil.dip2px(context, 3), AppInfoUtil.dip2px(context, 0));
			...
			for (int i = 0; i < dotViewsList.size(); i++) {
				if (i == pos) {
					((View) dotViewsList.get(pos))
							.setBackgroundResource(R.drawable.carousel_point_select);
				} else {
